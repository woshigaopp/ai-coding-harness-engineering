...
- `Source design object` 必须覆盖 AIP-owned locked ADEC/DEC、`MECH-*`、`FACT-*`、`CONSTRAINT-*`、AIP 当时 Current Architecture Understanding 的关键 evidence、接口/状态/runtime/兼容/验证决策。
- readiness 之后由 design/archaeology/migration/frontend/verification/task-planning 新增的阶段决策不得反向成为 AIP materialization obligation；只有下游证据证明 AIP-owned 决策或机制缺失/错误时才创建 AIP backflow。
