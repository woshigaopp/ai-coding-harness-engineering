...
`Mechanism-Level Design Closure Matrix` 用来证明设计问题已经从概念层降到机制层。`AIP Narrative Materialization Gate` 用来证明 sidecar 中的 ADEC/DEC、外部机制事实、当前架构证据、接口/状态/runtime/兼容/验证决策已经写入 AIP 正文，而不是只留在机器可读矩阵里。
